...
| Рендер | Google Filament `1.74.0` как временный OpenGL ES bridge; native Vulkan V2.1 offscreen runtime | Filament сохраняет рабочий camera/makeup baseline; его Vulkan camera streams отвергнуты device proof. Persistent native device/swapchain/render-pass lifecycle реализован отдельно и ожидает device acceptance. |
| Native GPU toolchain | NDK r29 `29.0.14206865`, CMake `3.31.6`, C++20 | JNI/Vulkan frame graph, строгая компиляция четырёх ABI с `-Werror`; V2.1 dynamic dispatch и persistent resources подключены. |
