...
4. **V6 / Temporal quality — активный обязательный этап:** визуально стабильный V6.2 robust/quality-aware baseline сохранён commit `cc82370` и тегом `tracking-v6.2-stable-2026-08-17`. Поверх него принят вариант с плавным quality/speed/confidence-gated alignment horizon `45→85 ms` и отдельным тегом `tracking-v6.2-fast-motion-2026-08-17`: резкое движение стало лучше, device analyzer сообщил lag `0 ms`, stop overshoot не вырос, но небольшой общий lag остаётся. Следующий slice разделяет геометрический lag и temporal mismatch camera-conditioned материала, добавляет finish/GPU timing telemetry и motion-aware material coherence. После этого нужны controlled light/thermal replay A/B. Vulkan V4 flow остаётся невидимым и не выполняет temporal compute вне диагностической записи.
