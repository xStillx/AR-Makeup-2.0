...
- Реализованы диагностический camera/ML pipeline, зафиксированный tracking baseline, Vulkan V4 temporal-compute vertical slice и V5.0 camera-conditioned material foundation с реконструированными нормалями и профилями matte/satin/gloss. Semantic face parsing и финальная production BRDF/HDR-калибровка ещё не добавлены.
...
4. **Temporal quality — текущий главный незавершённый этап:** Vulkan V4 flow работает только как GPU telemetry и не изменяет видимую lip mesh; активным остаётся `LandmarkMotionPredictor`, который является лишь сравнительным baseline и тоже требует переработки. До следующего material/semantic этапа нужны replayable telemetry, измерение stationary jitter/lag/overshoot, калибровка global pose и local deformation, записанное динамическое A/B, multi-device/thermal проверка. Повторное включение flow разрешено только после измеренного превосходства над улучшенным predictor.
5. **Face parsing — следующий V5-подэтап:** выбрать и документально проверить коммерческую лицензию модели и датасета для skin/lips/eyes/eyelids; только затем LiteRT GPU/NPU/CPU benchmark, вероятностная lip mask и temporal warp.
