...
Совместный shadow vertical slice `FF1/FF2` уже реализован: latency telemetry расширена до CPU-observable render timeline, facial transformation matrix записывается с camera timestamp, а видимая lip mesh не изменена. Ближайшие действия — получить настоящий presentation timing через Perfetto/FrameTimeline, завершить controlled matrix-сценарии и сравнить transform continuity с текущим 22-anchor estimator/gyro. Переход к видимому 3D path до этого запрещён.
...
- Roadmap перехода от текущего lip-focused candidate к model-independent full-face 3D tracking принят и вынесен в `FULL_FACE_ROADMAP.md`; `FF0` завершён, `FF1/FF2` выполняются в shadow/telemetry-режиме, `FF3+` ещё не начаты.
- Git checkpoints: V6.0 — commit `8d48b46` (`[V6]`); визуально стабильный V6.2 baseline — commit `cc82370` и tag `tracking-v6.2-stable-2026-08-17`; принятый fast-motion вариант — commit `d4636ac` и tag `tracking-v6.2-fast-motion-2026-08-17`; motion-aware material candidate — commit `ab8796a` и tag `material-temporal-v1-candidate-2026-08-17`; V6.3 gyro experimental — commit/tag `dd095f7` / `tracking-v6.3-gyro-experimental-2026-08-17`; FF1/FF2 shadow telemetry — `0554924`; license registry — `d083af2`; canonical transform contract — `19a2280`; FF1 render timeline v7 — `3419063`.
...
Следующий FF1 шаг — записать actual presentation через Perfetto/FrameTimeline и сопоставить его с v7 CPU markers; `cameraFrameSelected` и `renderSubmit` нельзя называть presentation. Параллельно для FF2 всё ещё нужны face-visible yaw/pitch, stop/dropout/weak-light/thermal runs и сравнение matrix continuity с 22-anchor/gyro по sensor/display timestamps.
