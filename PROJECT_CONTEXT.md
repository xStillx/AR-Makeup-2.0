...
- Реализованы диагностический camera/ML pipeline, сравнительный tracking baseline, Vulkan V4 temporal-compute shadow vertical slice, V5.0 camera-conditioned material foundation и V6.0 measurement foundation с записываемой `.arv6` telemetry/replay. Следующий подэтап V6 — холодная scenario matrix и измеримое A/B нового stateful global/local predictor; semantic face parsing и финальная production BRDF/HDR-калибровка ещё не добавлены.
- Git checkpoint V6.0: commit `8d48b46` (`[V6]`) находится в `master` и `origin/master`; при проверке 2026-08-14 ветки были синхронизированы (`0 ahead / 0 behind`), рабочее дерево было чистым до этого обновления документации.
...
Каждый ML-result сохраняет все raw 478×xyz landmarks, base positions и velocities текущего predictor до render correction, capture sensor/uptime timestamp, delivery timestamp, capture→delivery latency, rolling ML FPS, face-present/dropout и predicted-only. Публичный `FaceLandmarkerResult` используемой версии не отдаёт один калиброванный face-confidence, поэтому это поле записывается как `NaN`, а не заполняется выдуманной оценкой. Дополнительно сохраняется диагностическое разложение global pose/local deformation: translation берётся по жёстким eye/nose/cheek anchors, scale/rotation — по межглазному вектору, а 40 точек внешнего+внутреннего lip contour переводятся в head-local координаты. Это разложение пока только измеряется и не двигает mesh.
...
4. **V6 / Temporal quality — активный обязательный этап:** V6.0 replayable telemetry и первые метрики реализованы и зафиксированы commit `8d48b46`; Vulkan V4 flow работает только как GPU telemetry и не изменяет видимую lip mesh, а `LandmarkMotionPredictor` остаётся сравнительным baseline. Для V6.1 записать холодную scenario matrix, затем реализовать и на одинаковых `.arv6` сравнить разделение global pose/local deformation и состояния покоя/движения/остановки/dropout. Повторное включение flow разрешено только после измеренного превосходства над улучшенным predictor.
5. **Face parsing — после V6:** выбрать и документально проверить коммерческую лицензию модели и датасета для skin/lips/eyes/eyelids; только затем LiteRT GPU/NPU/CPU benchmark, вероятностная lip mask и temporal warp.
