...
- Реализованы диагностический camera/ML pipeline, Vulkan V4 temporal-compute shadow vertical slice, V5.0 camera-conditioned material foundation, V6.0 `.arv6` telemetry/replay, V6.1 stateful global/local predictor и первый device-проверенный срез V6.2: robust similarity fit, quality-aware prediction и телеметрия освещения/exposure/frame pacing/thermal. Пользователь принял стабильность V6.2 без заметного jitter, но при резких движениях остаётся отставание; текущая версия должна быть сохранена отдельным rollback checkpoint до ускорения fast-motion response. Semantic face parsing и финальная production BRDF/HDR-калибровка ещё не добавлены.
- Git checkpoint V6.0: commit `8d48b46` (`[V6]`); актуализированный контекст зафиксирован commit `cd1a560` (`[UpdateContext]`). В начале работы 2026-08-17 `master` и `origin/master` были синхронизированы на `cd1a560`.
...
4. **V6 / Temporal quality — активный обязательный этап:** V6.0 replayable telemetry зафиксирована commit `8d48b46`; V6.1 устранил forward overshoot. Первый V6.2 slice заменил восьмиточечный pose на 22-anchor Huber-robust similarity fit, сделал prediction quality-aware и расширил telemetry до illumination/exposure/frame pacing/pose quality/thermal. Локальный gate, device proof и visual stability acceptance пройдены; осталось уменьшить lag при резких движениях без возврата overshoot/jitter, после чего провести контролируемые light/thermal replay A/B. Vulkan V4 flow остаётся невидимым и не выполняет temporal compute вне диагностической записи.
