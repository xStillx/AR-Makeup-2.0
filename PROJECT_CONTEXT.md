...
Совместный shadow vertical slice `FF1/FF2` уже реализован: latency telemetry расширена до CPU-observable render timeline и Choreographer FrameTimeline correlation, facial transformation matrix записывается с camera timestamp, а видимая lip mesh не изменена. На SM-G990B actual-present текущего Filament `SurfaceView` получен отдельным SurfaceFlinger layer-sidecar: smooth 60 Hz output всё равно появляется примерно на `44 ms` позже requested/desired presentation. Ближайшие действия — controlled A/B низколатентного frame scheduling/presentation и оставшиеся matrix-сценарии; ещё раз расширять predictor без учёта display target нельзя. Переход к видимому 3D path до FF1/FF2 gate запрещён.
