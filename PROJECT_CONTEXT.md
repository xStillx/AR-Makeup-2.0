...
- Реализованы диагностический camera/ML pipeline, сравнительный tracking baseline, Vulkan V4 temporal-compute shadow vertical slice и V5.0 camera-conditioned material foundation с реконструированными нормалями и профилями matte/satin/gloss. Следующим запланирован V6 для переработки predictor и production temporal tracking; semantic face parsing и финальная production BRDF/HDR-калибровка ещё не добавлены.
...
4. **V6 / Temporal quality — следующий обязательный этап:** Vulkan V4 flow работает только как GPU telemetry и не изменяет видимую lip mesh; активный `LandmarkMotionPredictor` является лишь сравнительным baseline. Реализовать replayable telemetry, разделение global pose/local deformation, состояния покоя/движения/остановки/dropout, измерение jitter/lag/overshoot и записанное A/B с multi-device/thermal проверкой. Повторное включение flow разрешено только после измеренного превосходства над улучшенным predictor.
