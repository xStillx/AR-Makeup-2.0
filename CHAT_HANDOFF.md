...
- Ветка: `master`; стабильный V6.2 baseline зафиксирован commit `cc82370` и annotated tag `tracking-v6.2-stable-2026-08-17`. Улучшенный fast-motion вариант принят и сохранён commit `d4636ac` / tag `tracking-v6.2-fast-motion-2026-08-17`; старый baseline остаётся доступен для отката. Текущий незакоммиченный diff после `d4636ac` — только material-temporal coherence, `.arv6` v3 telemetry, тесты и контекст; predictor в нём не меняется.
...
В текущем незакоммиченном material candidate `LipstickMaterialTemporalController` измеряет скорость фактически загруженного lip contour и timestamp mismatch между camera buffer и predicted mesh. Оценка пространственного рассогласования плавно управляет новым shader uniform `cameraDetailCoherence`: `1→0.18` только для high-frequency camera texture/highlight/быстрой части light gradient; pigment, coverage, mesh и predictor неизменны. После gap/скрытия state сбрасывается; `TRACKING_TEST` всегда получает coherence `1`. `.arv6` version 3 обратно читает v1/v2 и записывает finish, coherence, motion speed, timestamp mismatch, frame-submission CPU time и `beginFrame` accepted; analyzer выдаёт per-finish CPU median/p95 и accepted fraction. Это proxy GPU backpressure, не точный GPU duration. Добавлены controller/codec/analyzer regressions; полный локальный gate прошёл: 99 unit-тестов, lint, debug APK и native build четырёх ABI.

Device gate выполнен на холодном SM-G990B. Material с новым uniform скомпилировался, camera `1440×1080`, `cameraDropped=0`, crash/black frame нет. `material_satin_v3_sharp_motion`: 599 ML / 1198 render, dropped 0, ML FPS 30.31, latency 102/116 ms, frame CPU 2.75/5.55 ms, rendered fraction 1.0; coherence при резком движении реально опускался до `0.18`. `material_finish_ab_v3`: 697 ML / 1441 render, dropped 0, все четыре finish и rendered fraction 1.0. Per-finish CPU median/p95: GLOSS 3.05/6.30 ms, MATTE 3.11/6.56, SATIN 3.12/6.62, TRACK 2.99/6.81 — profile-specific render slowdown не обнаружен. `gfxinfo` CPU p50/p90/p95/p99 9/11/12/13 ms, GPU 4/5/6/7 ms, jank около 0.22–0.25%, thermal max 0. Обе `.arv6` записи лежат в `app/build/tracking-telemetry`. Осталась пользовательская visual acceptance дополнительного шлейфа и сохранения естественной фактуры; приложение оставлено в режиме `SATIN`.
...
Начни новый чат с изучения `AGENTS.md`, `PROJECT_CONTEXT.md` и перечисленных tracking/render-файлов. Baseline `cc82370` / `tracking-v6.2-stable-2026-08-17` визуально стабилен; fast-motion `d4636ac` / `tracking-v6.2-fast-motion-2026-08-17` принят как улучшенный checkpoint, хотя небольшой общий lag сохраняется. Поверх него реализован незакоммиченный material-temporal candidate; локальный и device/runtime/performance gate пройдены. Следующий шаг — пользовательская visual acceptance `SATIN/MATTE/GLOSS` против `TRACKING_TEST`: проверить дополнительный шлейф, размытие/пульсацию фактуры и восстановление блика после остановки. Predictor в этом A/B не менять.
