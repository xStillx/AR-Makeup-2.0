...
Camera-value color transfer реализован для MATTE, SATIN и GLOSS в общем GLSL: hue/saturation берутся из pigment, value — из пикселя камеры той же exact Sync-пары с масштабом pigmentValue/0.85. Новый ползунок Яркость из камеры имеет диапазон 0–1 и позволяет сравнить прежний путь (0) с полным переносом (1); default 1 сохранён раздельно для каждого finish. Banuba SDK, модели, LIPS/LIPS_SHINING masks и assets не добавлены; стандартные RGB↔HSV функции написаны в проекте самостоятельно.

:app:assembleDebug после color transfer и управления цветом успешен, unit-тесты не запускались. Новая сборка ещё не установлена: на момент проверки ADB-устройств не было. Нужны runtime-компиляция GLSL и визуальный A/B на Samsung для всех трёх finish. При проверке держать LipFrameTrace выключенным. Рабочее дерево содержит незакоммиченные изменения; commit/push не запрошены.
