...
Актуально на 2026-09-10. Это forward-only roadmap. Текущее состояние, измерения и итоги завершённых исследований находятся в `PROJECT_CONTEXT.md`.
...
Для визуальной итерации добавлена сворачиваемая панель 35 material/color/coverage/camera-sampling параметров. Она нужна для контролируемого подбора на одном и том же Sync-пайплайне и экспорта точных значений вместо правок коэффициентов вслепую. SATIN Dior 999 и GLOSS Brown Espresso 515 сохранены как утверждённые defaults; стартовый camera-value transfer всех product finish выбирается по HSV value пигмента (0.82/0.90/1.00 для тёмного/среднего/светлого диапазона). Индивидуальные финальные параметры продуктов имеют приоритет. После visual acceptance панель следует убрать или ограничить перед production.
## Завершено для Dior 999; следующие проверки — остальные finish и производительность

Исторически пользователь 2026-09-04 остановил первый перенос после linear compositing. После исправления внутренней coverage, добавления двухпроходного alpha blur и интерактивного подбора пользователь 2026-09-10 объявил новый набор Dior Rouge Dior Satin 999 конечным. Результаты исследования iOS сохранены в [IOS_REFERENCE_CONTEXT.md](IOS_REFERENCE_CONTEXT.md).

Текущий SATIN preset и мягкий край зафиксированы. Открыты проверка общего blur на MATTE/GLOSS, performance benchmark и решение, нужен ли expanded outer carrier; эти пункты не должны менять утверждённый Dior preset без новой команды.
...
2. Проверить усиленный двухпроходный 13-tap alpha blur: базовый scale 1.5× и outer feather 25%; ползунок Растушёвка края доводит их до 2.5×/45%. Фактура/RGB сохраняются, рот защищён текущей mesh, fragment work ограничен lip ROI. После A/B решить, нужен ли внешний carrier/SDF gate для небольшого хвоста за outer contour.
...
Gate Dior 999 закрыт явным выбором конечных параметров пользователя. Расширенная проверка MATTE/GLOSS, длительная проверка разговора/поворотов и performance остаются отдельными gate. Наличие iOS live prediction, ARKit closure veto или ML-training папки не разрешает переносить их в Android.
