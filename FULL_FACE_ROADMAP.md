...
- `.arv6` v10 проверил Filament presentation hints. Их отключение ухудшило desired→actual `43.88→59.27 ms` и ready→actual `39.77→49.64 ms`; default hints-on обязателен, candidate отклонён.
- Актуальный локальный gate: `131` unit test, `0` failures/errors, lint и debug APK/четыре ABI успешны. Predictor, geometry, camera transforms, materials, зависимости и модели не менялись.
- До завершения FF1 нужен native Vulkan visible swapchain/present proof с presentation feedback и controlled A/B против Filament baseline; до завершения FF2 — face-visible yaw/pitch, stop/dropout/light/thermal matrix runs и controlled continuity/overhead acceptance.
...
1. Зафиксировать отклонённые Filament queue/hints candidates отдельным checkpoint; default остаётся queue protection on + presentation hints on.
2. Спроектировать минимальный native Vulkan visible swapchain/present vertical slice: тот же camera AHB, fences и проверенные transforms, но собственная surface ownership, latest-only image selection и actual-present feedback. Не переносить full-face/material архитектуру одновременно.
3. Сначала показать camera + текущую tracking-test lip geometry через Vulkan без regressions orientation/mirror/crop; сохранить debug A/B Filament/Vulkan и измерять sensor→actual, present queue, interval/jank, CPU/GPU и thermal.
