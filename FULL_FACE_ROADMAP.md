...
- Commit `082d88e` поднимает `.arv6` до v11 и записывает для native exact displayed contours/predictor/gyro state, backend, `presentID` и сопоставленный actual-present. Формат читает v1–v10; локально пройдены `137` tests, lint, APK и четыре ABI. Первый functional run дал `907` renders, `dropped=0` и actual-present coverage `0.99559`, но вариативность SurfaceFlinger queue `30→47 ms` требует exact cold repeat и запрещает прежний вывод о гарантированно более короткой native queue.
...
1. Подключить SM-G990B, установить exact APK commit `2fb7cd7` и выполнить functional gate retained-texture path: first frame, orientation/mirror/crop/lip alignment, `cameraDropped=0`, copy/present/reuse counters и lifecycle.
2. Записать controlled telemetry-on Filament/Vulkan A/B: direct sensor→actual, desired/ready→actual, actual interval/jank, duplicated frames, CPU/GPU/thermal и head/phone-motion visual response. Gate: около `16.7 ms` interval, `retainedPresents ≈ 2×cameraCopied`, reuse fraction около `0.5`; rollback `9bc3efb`.
