...
- Последующие predictor/gyro/flow и face-wide affine/projective A/B не устранили одновременно head-motion lag, phone-motion slippage, jitter и искажение локального контура. Принято другое разделение ответственности: ARCore владеет front-camera session, camera timeline и current global mouth anchor, MediaPipe получает latest-only image той же session и восстанавливает local 2D lip shape.
- Device visual acceptance на SM-G990B пройдена для timestamped translation transport: ARCore anchor не слетает при резких движениях головы/устройства, а MediaPipe сохраняет локальную мимику. Диагностические landmark-точки выключены; при pitch `24°` маска скрывается с трёхкадровым подтверждением и возвращается ниже `18°`. Эта Activity назначена обычным launcher и является основным tracking baseline; legacy CameraX/Filament path сохранён как rollback.
- Основной OpenGL ES launcher теперь включает принятый material foundation: XML/View-переключатели Matte/Satin/Gloss/Track, повторное семплирование того же ARCore camera texture внутри lip mesh, coverage/normals/lip-UV и camera-conditioned linear-RGB shading. Product sRGB pigment явно переводится в linear RGB; после исправления пользователь принял насыщенность всех режимов. Локальный gate: `162` unit-теста, `0` failures/errors, lint и debug APK; GLSL успешно собран Adreno 660. Production Vulkan cutover, full-face contract, semantic refinement, compatibility fallback и multi-device thermal gate ещё не выполнены.
...
Текущий status 2026-08-26: latency decomposition, native/Filament present matrix и predictor/gyro/flow эксперименты завершены как исследовательская база. ARCore + MediaPipe timestamped-anchor baseline и его OpenGL ES camera-conditioned Matte/Satin/Gloss/Track material foundation визуально приняты и назначены основным launcher. Решение ещё не production-complete: нет model-independent full-face contract, semantic boundary refinement, production Vulkan/BRDF/HDR, compatibility fallback и multi-device thermal gate. Следующий этап начинается с FF3 contract и затем переносит неизменённую принятую tracking/material-семантику в Vulkan timeline.
...
Статус 2026-08-26: интерфейсы ещё не введены, но принятый launcher baseline подтвердил обязательность этой границы. Контракт должен поддерживать одновременно ARCore global pose/mesh и MediaPipe local observations из одного camera timeline, а также fallback backend для устройств без ARCore, не меняя визуально принятую timestamped-anchor семантику.
...
Статус 2026-08-26: основной launcher baseline проходит пользовательские резкие head/phone сценарии без отдельного IMU/flow fusion; predictor и gyro не входят в видимый путь. Production FF4 должен перенести timestamped ARCore anchor transport в общий render state без визуальной регрессии, сохранить локальную мимику MediaPipe и подтвердить jitter/thermal/dropout/fallback на полном makeup renderer.
...
Основой дальнейшей работы является принятый 2026-08-26 ARCore + MediaPipe timestamped-anchor launcher baseline.
...
3. Перенести hybrid state в production Vulkan camera/render timeline, воспроизвести визуально принятые Matte/Satin/Gloss профили и затем завершить полный color/HDR contract. Текущий OpenGL ES launcher material baseline оставить точкой визуального сравнения и отката.
