...
- Checkpoint `80aaf7e` сохраняет device fixes: новая runtime/surface generation сбрасывает retained-camera timestamps и один раз восстанавливает Vulkan runtime после present failure, поэтому Home/resume больше не приводит к `Vulkan retained-camera present failed` и чёрному экрану. Local MediaPipe geometry принимается не старше `120 ms`, плавно ослабляется по yaw/pitch и affine residual (`full <= 0.006`, reject `> 0.016`), а при краткой потере удерживается до `100 ms`. При мимике lips больше не перепривязываются к неподвижному ARCore midpoint: stable-anchor affine сохраняет локальную трансляцию рта вместе с формой, тогда как глобальное head/phone motion остаётся исключительно ARCore-owned. Пользователь принял резкие head/phone движения, открытый рот, большие повороты и боковую мимику как текущий checkpoint; у экстремальных yaw/pitch сохраняется небольшой 2D perspective/occlusion residual. Локальный gate: `173` unit-теста, lint, APK и native build четырёх ABI; APK SHA-256 `2A285DC292E07903165443C21674E2B5BC5C75FDAD229DABE4A34E25AFB5A996`. Полный 10–15-минутный thermal soak ещё не выполнен.
- Первый FF5 candidate добавляет immutable `FaceSurfaceTopology`, ARCore projected NDC depth, per-swapchain Vulkan depth attachments, depth-only 468-point face occluder и depth-tested lip mesh. Технический gate: `177` unit-тестов, lint, APK и четыре ABI; APK SHA-256 `013758091AA4D2C3A0E718BD22744B9955F488A0DC2B537D70BDE27924E2F9EA`. На SM-G990B выбран `D32_SFLOAT`, `faceDepthUpdates` растёт, Home/resume проходит без black screen. Visual gate отклонён: forward drift на сильном yaw, участки помады проваливаются в губы и открывают кожу, при сильном pitch вниз маска съезжает и мигает. Принятый rollback остаётся `80aaf7e`.
...
1. Использовать `80aaf7e`/`92fc514` как принятый 2D Vulkan baseline, а сохранённый FF5 commit — только как отклонённый 3D candidate для сравнения.
