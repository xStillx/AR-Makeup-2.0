...
Для следующей визуальной итерации добавлена сворачиваемая панель всех доступных material/coverage/camera-sampling параметров. Она нужна для контролируемого подбора на одном и том же Sync-пайплайне и экспорта точных значений вместо правок коэффициентов вслепую. Настройки разделены по finish; выбранные пользователем значения записаны как SATIN defaults, matte/gloss пока нейтральны. Следующая задача — camera-value color transfer для всех вариантов помады по открытому Banuba shader reference, без Banuba SDK, моделей и masks. После visual acceptance панель следует убрать или ограничить перед production.
